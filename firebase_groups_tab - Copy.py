...
class ModernGroupsTab:
    ...
    def create_library_item(self, parent, item_id, item_data, approved=True):
        ...
        tk.Label(inner, text=icon, font=('Segoe UI', 14), bg='white').pack(side=tk.LEFT, padx=(0, 8))
        ...
        if self.is_admin_or_creator():
            if approved:
                ...
                pin_btn = tk.Label(actions, text=pin_text, font=('Segoe UI', 9),
                                   bg=self.colors['secondary'], fg='white', cursor='hand2', padx=10, pady=4)
                pin_btn.pack(side=tk.TOP, pady=2)
                pin_btn.bind('<Button-1>', lambda e, iid=item_id, current=item_data.get('pinned', False):
                             self.toggle_library_pin(iid, not current))

                remove_btn = tk.Label(actions, text="Remove", font=('Segoe UI', 9),
                ...
                approve_btn = tk.Label(actions, text="Approve", font=('Segoe UI', 9, 'bold'),
                                       bg=self.colors['secondary'], fg='white', cursor='hand2', padx=10, pady=4)
                approve_btn.pack(side=tk.TOP, pady=2)
                approve_btn.bind('<Button-1>', lambda e, iid=item_id: self.update_library_status(iid, 'approved'))

                reject_btn = tk.Label(actions, text="Reject", font=('Segoe UI', 9),
                                      bg=self.colors['accent'], fg='white', cursor='hand2', padx=10, pady=4)
                reject_btn.pack(side=tk.TOP, pady=2)
                reject_btn.bind('<Button-1>', lambda e, iid=item_id: self.remove_library_item(iid))
        else:
            if not approved:
                tk.Label(actions, text="Awaiting approval", font=('Segoe UI', 8), bg='white', fg=self.colors['gray']).pack()
